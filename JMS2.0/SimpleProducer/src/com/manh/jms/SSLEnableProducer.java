 ...
 * are enabled
 */
public class SSLEnableProducer {
    private static final String KEY_STORE_PASSWORD = "test123";
    private static final String TRUST_STORE_PASSWORD = "test123";
    private static final String KEY_STORE_FILE_NAME = "E:/Maven-Project/SimpleProducer/src/myproducer.ks";
    private static final String TRUST_STORE_FILE_NAME = "E:/Maven-Project/SimpleProducer/src/myclient.ts";
    ...
    private static void createQueueSession(Connection connection)
    {
		...
		try {
			...
			Queue queue=SimpleProducer.getQueue(session);
			QueueSender queueSender= (QueueSender)session.createSender(queue);
			queueSender.send(session.createTextMessage("Hello World SIDDHANTA"));
			session.close();
			connection.close();
		} catch (JMSException e) {
			e.printStackTrace();
    ...
    public static void main(String[] args) throws Exception 
    {
    	...
    	System.out.println("Stopping SSL ActiveMQ broker...");
    }
}
